...
$namespace('org.owasp.esapi.codec');

org.owasp.esapi.codec.HashTrie = function() {
    return {
        Entry: function(sKey,oVal) {
            var key = sKey;
            var val = oVal;
            ...
            return {
                ...
                equals: function(oEntry) {
                    return (oEntry.getKey && oEntry.getKey() === key) && (oEntry.getValue && oEntry.getValue() === val);
                },
        ...
        newEntryInstanceIfNeeded: function(sKey,oVal,nKeyLength) {
            if ( sKey == null || oVal == null ) {
                return null;
            }
            if ( nKeyLength ) {
                if ( sKey.length > nKeyLength ) {
                    sKey = sKey.substring(0,nKeyLength);
                }
            }
            return new Entry(sKey,oVal);
        },
        ...
        Node: function() {
            ...
            return {
                ...
                put: function(sKey,nPos,oValue) {
                    ...
                    else if ((nextNode = nextMap[ch]) == null) {
                        nextNode = new Node();
                        nextMap[ch] = nextNode;
                    }
                    return nextNode.put(sKey,nPos+1,oValue);
                },

                get: function(sKey,nPos) {
                    var nextNode;
                    if ( sKey.length <= nPos ) {
                        return value;
                    }
                    if ((nextNode = this.getNextNode(sKey.charAt(nPos)))==null) {
                        return null;
                    }
                    return nextNode.get(sKey,nPos+1);
                },

                getLongestMatch: function(sKey,nPos) {
                    var nextNode, ret;
                    if ( sKey.length <= nPos ) {
                        return org.owasp.esapi.codec.HashTrie.newEntryInstanceIfNeeded(sKey,value);
                    }
                    if ((nextNode = this.getNextNode(sKey.charAt(nPos)))==null) {
                        return org.owasp.esapi.codec.HashTrie.newEntryInstanceIfNeeded(sKey,nPos,value);
                    }
                    if ((ret = nextNode.getLongestMatch(sKey,nPos+1))!=null) {
                        return ret;
                    }
                },

                getLongestMatchPB: function( oPushbackString, sKey ) {
...
org.owasp.esapi.codec.HTMLEntityCodec = function() {
    var base = new Codec();

    base.encodeCharacter = function( oPushbackString ) {
        
    };

    base.getNamedEntity = function( oPushbackString ) {
        var possible = '', len;
        len = Math.min( oPushbackString.remainder().length,  )
    };

    return base;
};

org.owasp.esapi.codec.HTMLEntityCodec.characterToEntityMap = [];
org.owasp.esapi.codec.HTMLEntityCodec.entityToCharacterMap = [];
...
for ( var i = 0; i < entityNames.length; i ++ ) {
    var e = entityNames[i];
    var c = entityValues[c];
    org.owasp.esapi.codecs.HTMLEntityCodec.entityToCharacterMap[e] = c;
    org.owasp.esapi.codecs.HTMLEntityCodec.characterToEntityMap[c] = e;
}